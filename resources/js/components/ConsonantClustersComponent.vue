...
export default {
  ...
  data() {
    return {
      ...
      score: 0,
      rules:
        "Enter words that have only one consonant cluster. That is multiple consonants coming one after another",
      listOfPlayerWords: [],
      computer: "",
      timer: 10,
      myTimer: null,
  ...
  methods: {
    ...
    endLevel() {
      ...
      if (this.level == 2) {
      this.rules =
        "Enter words that have two consonant clusters.";
      } else if (this.level == 3) {
        this.rules =
        "Enter words that have three consonant clusters.";
      }
      if (this.level == 4) {
